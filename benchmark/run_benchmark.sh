...
submit_training_then_benchmark() {
    ...
    sbatch \
        ...
        --export=ALL,CHECKPOINT_PATH="${CHECKPOINT_PATH}",TEST_DIR="${TEST_DIR}",OUTPUT_DIR="${OUTPUT_DIR}",RUN_ID="${RUN_ID}",ENV_NAME="${ENV_NAME}",ROOT_REQUIREMENTS="${ROOT_REQUIREMENTS}",ATTACK_FILTER="${ATTACK_FILTER}",CASE_FILTER="${CASE_FILTER}",MAX_CASES="${MAX_CASES}",T_STAR="${T_STAR}",BATCH_SIZE="${BATCH_SIZE}",SEED="${SEED}",LOG_STEPS="${LOG_STEPS}",SKIP_PURIFY="${SKIP_PURIFY}",SKIP_RETRAIN="${SKIP_RETRAIN}",OVERWRITE_ARTIFACTS="${OVERWRITE_ARTIFACTS}",WB_EPOCHS="${WB_EPOCHS}",WB_DRYRUN="${WB_DRYRUN}",BP_VICTIM_NET="${BP_VICTIM_NET}",BP_CHECKPOINT_NAME="${BP_CHECKPOINT_NAME}",BP_RETRAIN_EPOCHS="${BP_RETRAIN_EPOCHS}",BP_RETRAIN_BSIZE="${BP_RETRAIN_BSIZE}",NS_PROFILE="${NS_PROFILE}",NS_VICTIM_EPOCHS="${NS_VICTIM_EPOCHS}",NS_VICTIM_BATCH_SIZE="${NS_VICTIM_BATCH_SIZE}",NS_VICTIM_WORKERS="${NS_VICTIM_WORKERS}",NS_VICTIM_SEED="${NS_VICTIM_SEED}",NS_VICTIM_CHECKPOINT_INTERVAL="${NS_VICTIM_CHECKPOINT_INTERVAL}",NS_VICTIM_RESUME="${NS_VICTIM_RESUME}",NS_TRIGGERED_TEST_LIMIT="${NS_TRIGGERED_TEST_LIMIT}",PURIFY_CACHE_DIR="${PURIFY_CACHE_DIR}" \
        "${self_script}"
...
NS_VICTIM_EPOCHS="${NS_VICTIM_EPOCHS:-}"
NS_VICTIM_BATCH_SIZE="${NS_VICTIM_BATCH_SIZE:-128}"
NS_VICTIM_WORKERS="${NS_VICTIM_WORKERS:-8}"
NS_VICTIM_SEED="${NS_VICTIM_SEED:-62000}"
NS_VICTIM_CHECKPOINT_INTERVAL="${NS_VICTIM_CHECKPOINT_INTERVAL:-1}"
NS_VICTIM_RESUME="${NS_VICTIM_RESUME:-1}"
NS_TRIGGERED_TEST_LIMIT="${NS_TRIGGERED_TEST_LIMIT:-}"
PURIFY_CACHE_DIR="${PURIFY_CACHE_DIR:-}"
...
BENCHMARK_ARGS=(
    ...
    --ns-victim-batch-size "${NS_VICTIM_BATCH_SIZE}"
    --ns-victim-workers "${NS_VICTIM_WORKERS}"
    --ns-victim-seed "${NS_VICTIM_SEED}"
    --ns-victim-checkpoint-interval "${NS_VICTIM_CHECKPOINT_INTERVAL}"
)
...
if [[ -n "${NS_VICTIM_EPOCHS}" ]]; then
    BENCHMARK_ARGS+=(--ns-victim-epochs "${NS_VICTIM_EPOCHS}")
fi
if [[ -n "${NS_TRIGGERED_TEST_LIMIT}" ]]; then
    BENCHMARK_ARGS+=(--ns-triggered-test-limit "${NS_TRIGGERED_TEST_LIMIT}")
fi
if [[ "${NS_VICTIM_RESUME}" != "1" ]]; then
    BENCHMARK_ARGS+=(--ns-no-victim-resume)
fi
if [[ -n "${PURIFY_CACHE_DIR}" ]]; then
    BENCHMARK_ARGS+=(--purify-cache-dir "${PURIFY_CACHE_DIR}")
fi
